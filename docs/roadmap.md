# Roadmap

## Near term

- improve Hugging Face Trainer state extraction
- add persisted run stores
- support notebook cell execution hooks
- expand fine-tuning metric aliases for LoRA, CLIP, projector, and VLM runs
- add more notebook examples

## Mid term

- Accelerate and DeepSpeed log adapters
- PEFT/LoRA adapter diagnostics
- CLIP/SigLIP bidirectional retrieval analyzers
- ViT augmentation and layer-wise LR diagnostics
- projector-only and staged VLM fine-tuning analyzers
- entry-point plugin discovery
- local LLM provider adapter

## Long term

- experiment lineage across notebooks
- automated train/eval leakage checks for instruction and image-text datasets
- contributor plugin catalog
